...
- `bun run migrate` — run database migrations from `migrations/` (node-pg-migrate, TypeScript, `--decamelize`, hardcoded `socialstuffs` schema; reads `DATABASE_URL` from `.env`)
