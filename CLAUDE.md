...
4. Serves static assets from `static/` — `/js/*` (vendored `htmx.min.js`, `nav.js`, `flash.js`) and the root favicon/manifest/robots files — via `staticCache` + `serveStatic`, scoped to those paths. `staticCache` sets `Cache-Control` (`public, max-age=2592000` — 30 days — in production, `no-store` in development) only on 200/304 static responses, so dynamic pages never inherit it.
5. Registers routes via `Routes()` (`server/routes/index.ts`), which delegates to route-group files like `public-routes.ts`. New route groups get their own file and are registered in `routes/index.ts`. Auth endpoints (`/sign-in`, `/sign-up`, `/validate-account`, `/resend-validation`) are wrapped with the per-IP rate limiter in `server/middleware/rate-limit-middleware.ts` (in-memory fixed-window; keys on the **last** `X-Forwarded-For` entry since the production ngrok tunnel appends the real client IP).
